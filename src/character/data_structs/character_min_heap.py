...
class CharacterHeap:
    ...
    def __init__(self, initial_characters: List[Character] = None, *, config: CharacterHeapConfig = None):
        self.config = config or CharacterHeapConfig(
            heapify_attribute="hp",
            order="min"
        )
    ...
    def insert(self, char: Character):
        """
        Insert based on character HP attribute
        """
        self._heap.append(char)
        idx = len(self._heap) - 1

        # compare to parent node
        while idx > 0 and self._attr(self._heap[(idx - 1) // 2]) > self._attr(self._heap[idx]):
            # parent attribute is greater, swap nodes
            self._heap[idx], self._heap[(idx - 1) // 2] = self._heap[(idx - 1) // 2], self._heap[idx]
            idx = (idx - 1) // 2
    ...
    def _heapify(self, i, n):
        ...
        if right < n and self._attr(self._heap[right]) < self._attr(self._heap[smallest]):
            smallest = right
